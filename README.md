# COMP90019
The purpose of this project is to mine opinions from Twitter by collecting and analysing tweets in near real-time over a distributed streaming platform. Sentiment analysis are applied on text found in tweets in conjunction with non-textual information e.g. emojis and images, that are peculiar in microblogging platforms. Tweets from trending topics are collected, pre-processed, analysed and then centrally stored for retrospective analysis. Over the last few years, deep learning has brought about new capabilities in image understanding e.g. classification, object detection, face detection. For social media platforms, mining textual information from images is important to track messages that spreads through images. However, creating a new deep learning algorithm is too intricate and time consuming for many who are interested in this field. It is much more efficient to combine pre-trained deep learning models that have been proven to accomplish the task. Here we have added deep learning libraries for text detection and recognition into our distributed streaming platform to extract text from images. Through data augmentation, by using additional features from non-textual information, we have built a real-time analytics platform that performs opinion mining on Twitter data which can be extended to any other social media platform.
